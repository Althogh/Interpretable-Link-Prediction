...
class TreeClassifier:
    ...
    def __initialize_sample_count__(self, X, y):  # thl
        """
        Compares every prediction y_hat against the labels y, then incorporates the misprediction into the stored loss values
        This is used when parsing models from an algorithm that doesn't provide the training loss in the out put
        """
        for node in self.__all_leaves__():
            node["sample_count"] = [0, 0]
            node['prob_predictions_1'] = 0.0
        (n, m) = X.shape
        for i in range(n):
            node = self.__find_leaf__(X.values[i, :])
            label = int(y.iloc[i].item())  # 使用 .iloc[i].item() 提取单个值并转换为整数
            node["sample_count"][label] += 1  # 增加该类别的样本计数
        # 计算每个叶节点的类别为1的概率
        for node in self.__all_leaves__():
            total_samples = node["sample_count"][0] + node["sample_count"][1]
            if total_samples > 0:
                node['prob_predictions_1'] = node["sample_count"][1] / total_samples
        return
    ...
    def predict_proba(self, X):# thl定义 predict_proba 方法来提取每个样本的 prob_predictions_1 值
        """
        Predicts the probability of each sample belonging to class 1 (prob_predictions_1).
        """
        prob_predictions = []
        (n, m) = X.shape
        for i in range(n):
            node = self.__find_leaf__(X.values[i, :])
            prob_predictions.append(node['prob_predictions_1'])
    ...
    def __str__(self):
        ...
        for group in self.__groups__():
            ...
            outcomes.append("    sample_count (0, 1): {}".format(group.get("sample_count", [0, 0])))  # thl打印出 0 和 1 的样本计数
            outcomes.append("    prob_predictions_1: {:6f}".format(group.get("prob_predictions_1", 0.0))) # thl打印出类别为1的概率
            result = "\n".join(outcomes)
